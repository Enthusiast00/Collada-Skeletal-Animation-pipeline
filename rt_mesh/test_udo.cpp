...
bool test_udo::Init(){
    ...
    if(! loader.Load("test-udo.dae")){ return false; }
    ...
    RT_HR_T(RT_D3D->GetD3dDevice()->CreateVertexBuffer(m_vertices.Size() * sizeof(rt_vertex_s),
    ...
    for(uint32_t i=0;i<m_vertices.Size();i++){
        ...
        v[i].bi = m_vertices[i].bi.x;
        v[i].bw = m_vertices[i].bw.x;
    }
    RT_HR_T(m_vb->Unlock());
...
//        m_sbmeshes[i].mat.specPower = 48.0f;

    }
    RT_HR_T(D3DXCreateTextureFromFile(RT_D3D->GetD3dDevice(),"test-udo_uv.bmp", &m_tex));
    AddFlags(RT_MESH_INIT);
    return true;
}

void test_udo::Update(){
    if(RT_EVENT.isevent){
        switch( RT_EVENT.msg ) {
        case WM_KEYUP : {
            ...
            if(RT_WIN->FlagTest(RT_WIN_LSHIFT) && !RT_WIN->FlagTest(RT_WIN_LCTRL)){
                if(RT_EVENT.wparam == 0x31){ Play(m_animinputs[0],m_animinputs[ 6 ]); }
                if(RT_EVENT.wparam == 0x32){ Play(m_animinputs[7],m_animinputs[44]);  }
            }
        }break;
        case WM_KEYDOWN : {
            if((RT_EVENT.wparam == VK_LEFT) && (RT_INST->GetCurrentModel()==0)){
                m_model_yaw = m_model_yaw< -D3DX_PI? m_model_yaw:m_model_yaw-0.1f;
                m_world= m_base* rtm_rotate(m_model_yaw,0.0f,0.0f,1.0f);
            }
            if((RT_EVENT.wparam == VK_RIGHT) && (RT_INST->GetCurrentModel()==0)){
                m_model_yaw = m_model_yaw> D3DX_PI? m_model_yaw:m_model_yaw+0.1f;
                m_world= m_base* rtm_rotate(m_model_yaw,0.0f,0.0f,1.0f);
            }
...
void test_udo::Draw(){
    ...
    RT_HR_E(RT_D3D->GetD3dDevice()->SetStreamSource(0,m_vb, 0, sizeof(rt_vertex_s)));

    RT_HR_E(RT_D3D->GetEffect()->SetTechnique(RT_D3D->Sdata().m_hatech_s));
    RT_HR_E(RT_D3D->GetD3dDevice()->SetVertexDeclaration(RT_D3D->GetSAnimDec()));
...
void  test_udo::Play(float st, float en){
    ...
    printf(" first bone index : %i last bone index : %i \n",m_fboneinx,m_lboneinx);
}
